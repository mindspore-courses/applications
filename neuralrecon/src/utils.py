...
def make_recursive_func(func):
    ...
    def wrapper(variables):
        ret = None
        if isinstance(variables, list): # pylint: disable R1705
            ret = [wrapper(x) for x in variables]
        elif isinstance(variables, tuple):
            ret = tuple([wrapper(x) for x in variables])
        elif isinstance(variables, dict):
            ret = {k: wrapper(v) for k, v in variables.items()}
        else:
            ret = func(variables)
        return ret
...
@make_recursive_func
def tensor2float(variables):
    """Tensor to float"""
    ret = None
    if isinstance(variables, float): # pylint: disable R1705
        ret = variables
    elif isinstance(variables, torch.Tensor):
        if not variables.shape:  # pylint: disable R1705
            ret = variables.data.item()
        else:
            ret = [v.data.item() for v in variables]
    else:
        raise NotImplementedError("invalid input type {} for tensor2float".format(type(variables)))
    return ret


@make_recursive_func
def tensor2numpy(variables):
    """Tensor to numpy array"""
    ret = None
    if isinstance(variables, np.ndarray):    # pylint: disable R1705
        ret = variables
    elif isinstance(variables, torch.Tensor):
        ret = variables.detach().cpu().numpy().copy()
    else:
        raise NotImplementedError("invalid input type {} for tensor2numpy".format(type(variables)))
    return ret


@make_recursive_func
def tocuda(variables):
    """To cuda"""
    ret = None
    if isinstance(variables, torch.Tensor):  # pylint: disable R1705
        ret = variables.cuda()
...
def save_scalars(logs, mode, scalar_dict, global_step):
    """Save scalars"""
    scalar_dict = tensor2float(scalar_dict)
    for key, value in scalar_dict.items():
        if not isinstance(value, (list, tuple)):
            name = '{}/{}'.format(mode, key)
            logs.add_scalar(name, value, global_step)
        else:
            for idx in range(len(value)):
                name = '{}/{}_{}'.format(mode, key, idx)
                logs.add_scalar(name, value[idx], global_step)


def save_images(logs, mode, images_dict, global_step):
    """Save images"""
    ...
    for key, value in images_dict.items():
        if not isinstance(value, (list, tuple)):
            name = '{}/{}'.format(mode, key)
            logs.add_image(name, preprocess(name, value), global_step)
        else:
            for idx in range(len(value)):
                name = '{}/{}_{}'.format(mode, key, idx)
                logs.add_image(name, preprocess(name, value[idx]), global_step)
...
class SaveScene():
    ...
    def vis_incremental(self, _, batch_idx, imgs, outputs):
        """Visualiza incremental"""
    ...
    def save_incremental(self, epoch_idx, batch_idx, _, outputs):
        """Save incremental"""
