...
from tools.tsdf_fusion.fusion_ms import TSDFVolumeNumpy


def coordinates(voxel_dim):
    ...
    Returns:
        numpy array of size (3,nx*ny*nz)
    """
...
class ToTensor():
    ...
    def __call__(self, data):
        data['imgs'] = np.stack(data['imgs']).transpose([0, 3, 1, 2])
        if 'depth' in data.keys():
            data['depth'] = np.stack(data['depth'])
        if 'tsdf_list_full' in data.keys():
            for i in range(len(data['tsdf_list_full'])):
                if not isinstance(data['tsdf_list_full'][i], np.ndarray):
                    data['tsdf_list_full'][i] = np.array(data['tsdf_list_full'][i], dtype=np.float32)
        return data


class IntrinsicsPoseToProjection():
    ...
    def __call__(self, data):
        ...
        data['world_to_aligned_camera'] = rotation_matrix4x4.astype(np.float32) @ np.linalg.inv(middle_pose)
        ...
        for intrinsics, extrinsics in zip(data['intrinsics'], data['extrinsics']):
            ...
            view_proj_matrics = np.stack(view_proj_matrics)
            proj_matrices.append(view_proj_matrics)
        data['proj_matrices'] = np.stack(proj_matrices)
        data.pop('intrinsics')
        data.pop('extrinsics')
        return data
...
class RandomTransformSpace():
    """ Apply a random 3x4 linear transform to the world coordinate system.
        This affects pose as well as TSDFs.
    """

    def __init__(self, voxel_dim=(48, 48, 48), voxel_size=1., random_rotation=True, random_translation=True,
        ...
        self.padding_start = np.array([paddingXY, paddingXY, paddingZ], dtype=np.float32)
        # no need to pad above (bias towards floor in volume)
        self.padding_end = np.array([paddingXY, paddingXY, 0], dtype=np.float32)

        # each epoch has the same transformation
        self.random_r = np.random.rand(max_epoch)
        self.random_t = np.random.rand((max_epoch, 3))

    def __call__(self, data):
        origin = data['vol_origin']
        if (not self.random_rotation) and (not self.random_translation):
            ...
            rotate = np.array([[np.cos(r), -np.sin(r)],
                               [np.sin(r), np.cos(r)]], dtype=np.float32)

            # get corners of bounding volume
            voxel_dim_old = np.array(data['tsdf_list_full'][0].shape) * self.voxel_size
            xmin, ymin, zmin = origin
            xmax, ymax, zmax = origin + voxel_dim_old

            corners2d = np.array([[xmin, xmin, xmax, xmax],
                                  [ymin, ymax, ymin, ymax]], dtype=np.float32)

            # rotate corners in plane
            corners2d = rotate @ corners2d
            ...
            start = np.array([xmin, ymin, zmin]) - self.padding_start
            end = - np.array(voxel_dim) * self.voxel_size + np.array([xmax, ymax, zmax]) + self.padding_end
        ...
        for i in range(len(data['extrinsics'])):
            data['extrinsics'][i] = trans @ data['extrinsics'][i]

        data['vol_origin'] = np.array(self.origin, dtype=np.float32)

        data = self.transform_data(data, np.linalg.inv(trans), old_origin=origin)

        return data

    def transform_data(self, data, transform=None, old_origin=None, align_corners=False):
        ...
        bnds = np.zeros((3, 2), dtype=np.float32)
        bnds[:, 0] = np.inf
        bnds[:, 1] = -np.inf
        ...
        for i in range(data['imgs'].shape[0]):
            ...
            bnds[:, 0] = np.minimum(bnds[:, 0], np.min(view_frust_pts, axis=1)[0])
            bnds[:, 1] = np.maximum(bnds[:, 1], np.max(view_frust_pts, axis=1)[0])

        # -------adjust volume bounds-------
        num_layers = 3
        center = (np.array(((bnds[0, 1] + bnds[0, 0]) / 2, (bnds[1, 1] + bnds[1, 0]) / 2, -0.2)) - data[
            'vol_origin']) / self.voxel_size
        center[:2] = np.round(center[:2] / 2 ** num_layers) * 2 ** num_layers
        center[2] = np.floor(center[2] / 2 ** num_layers) * 2 ** num_layers
        origin = np.zeros_like(center, dtype=np.float32)
        origin[:2] = center[:2] - np.array(self.voxel_dim[:2]) // 2
        origin[2] = center[2]
        ...
        if 'tsdf_list_full' in data.keys():
            # -------------grid coordinates------------------
            old_origin = old_origin.reshape(1, 3)

            x, y, z = self.voxel_dim
            coords = coordinates(self.voxel_dim)
            world = coords.astype(np.float32) * self.voxel_size + vol_origin_partial.reshape(3, 1)
            world = np.concatenate((world, np.ones_like(world[:1])), axis=0).astype(np.float32)
            ...
            for l, tsdf_s in enumerate(data['tsdf_list_full']):
                ...
                vol_dim_s = np.array(self.voxel_dim) // 2 ** l
                tsdf_vol = TSDFVolumeNumpy(vol_dim_s, vol_origin_partial,
                ...
                occ_vol = np.zeros_like(tsdf_vol).astype(bool)
                occ_vol[(tsdf_vol < 0.999) & (tsdf_vol > -0.999) & (weight_vol > 1)] = True

                # grid sample expects coords in [-1,1]
                coords_world_s = coords.reshape(3, x, y, z)[:, ::2 ** l, ::2 ** l, ::2 ** l] / 2 ** l
                dim_s = list(coords_world_s.shape[1:])
                coords_world_s = coords_world_s.reshape(3, -1)

                old_voxel_dim = list(tsdf_s.shape)

                coords_world_s = 2 * coords_world_s / (np.array(old_voxel_dim) - 1).reshape(3, 1) - 1
                coords_world_s = coords_world_s[[2, 1, 0]].T.reshape([1] + dim_s + [3]).astype(np.float32)

                # bilinear interpolation near surface,
                # no interpolation along -1,1 boundary
                tsdf_vol = ops.grid_sample(
                    Tensor(tsdf_s).view(([1, 1] + old_voxel_dim)),
                    Tensor(coords_world_s), interpolation_mode='nearest', align_corners=align_corners
                ).squeeze().asnumpy()
                tsdf_vol_bilin = ops.grid_sample(
                    Tensor(tsdf_s).view(([1, 1] + old_voxel_dim)),
                    Tensor(coords_world_s), interpolation_mode='bilinear', align_corners=align_corners
                ).squeeze().asnumpy()
                mask = np.abs(tsdf_vol) < 1
                ...
                mask = np.any(np.squeeze((np.abs(coords_world_s) >= 1), 0), axis=3)
                tsdf_vol[mask] = 1
...
def get_view_frustum(max_depth, size, cam_intr, cam_pose):
    ...
    view_frust_pts = np.stack([
        (np.array([0, 0, 0, im_w, im_w]) - cam_intr[0, 2]) * np.array(
