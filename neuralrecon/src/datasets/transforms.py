...
class RandomTransformSpace():
    ...
    def __call__(self, data):
        ...
        if (not self.random_rotation) and (not self.random_translation):
            ...
            rotate = torch.tensor([[np.cos(r), -np.sin(r)],
            ...
            end = - torch.Tensor(voxel_dim) * self.voxel_size + torch.Tensor([xmax, ymax, zmax]) + self.padding_end
        ...
        data['vol_origin'] = torch.tensor(self.origin, dtype=torch.float, device=trans.device)

        data = self.transform(data, trans.inverse(), old_origin=origin)
