...
FILETYPES = ['.aggregation.json', '.sens', '.txt', '_vh_clean.ply', '_vh_clean_2.0.010000.segs.json',
             '_vh_clean_2.ply', '_vh_clean.segs.json', '_vh_clean.aggregation.json', '_vh_clean_2.labels.ply',
             '_2d-instance.zip', '_2d-instance-filt.zip', '_2d-label.zip', '_2d-label-filt.zip']
...
def download_scan(scan_id, out_dir, file_types, use_v1_sens):
    ...
    for ft in file_types:
        ...
        url = BASE_URL + RELEASE + '/' + scan_id + '/' + scan_id + ft if not v1_sens else BASE_URL + \
              RELEASES[V1_IDX] + '/' + scan_id + '/' + scan_id + ft
...
def main():
    ...
    parser.add_argument('--preprocessed_frames', action='store_true', help=\
                        'download preprocessed subset of ScanNet frames (' + PREPROCESSED_FRAMES_FILE[1] + ')')
    parser.add_argument('--test_frames_2d', action='store_true', help='download 2D test frames (' + \
                        TEST_FRAMES_FILE[1] + '; also included with whole dataset download)')
    parser.add_argument('--type', help='specific file type to download (.aggregation.json, .sens, .txt, \
                        _vh_clean.ply, _vh_clean_2.0.010000.segs.json, _vh_clean_2.ply, _vh_clean.segs.json, \
                        _vh_clean.aggregation.json, _vh_clean_2.labels.ply, _2d-instance.zip, _2d-instance-filt.zip, \
                        _2d-label.zip, _2d-label-filt.zip)')
    args = parser.parse_args()

    print('By pressing any key to continue you confirm that you have agreed to the ScanNet terms of use as \
          described at:')
    print(TOS_URL)
    print('***')
    ...
    # out_dir_scans = os.path.join(args.out_dir, 'scans')     # 下载前是.sens文件 改放在raws下 解析后的放在scans下
    out_dir_scans = os.path.join(args.out_dir, 'raws')
    ...
    if args.task_data:  # download task data
        ...
        print('You are downloading the preprocessed subset of frames ' + PREPROCESSED_FRAMES_FILE[0] + \
              ' which requires ' + PREPROCESSED_FRAMES_FILE[1] + ' of space.')
        ...
        print('You are downloading the 2D test set ' + TEST_FRAMES_FILE[0] + ' which requires ' + \
              TEST_FRAMES_FILE[1] + ' of space.')
        ...
        if scan_id not in release_scans and (not is_test_scan or args.v1):
            ...
            out_dir = os.path.join(out_dir_scans, scan_id) if not is_test_scan else \
            ...
            if not is_test_scan and not args.v1 and '.sens' in scan_file_types:
                print('Note: ScanNet v2 uses the same .sens files as ScanNet v1: Press \'n\' to exclude downloading \
                      .sens files for each scan')
                # key = raw_input('')
        ...
        if len(file_types) == len(FILETYPES):
            print('WARNING: You are downloading the entire ScanNet ' + RELEASE_NAME + ' release which requires ' + \
                  RELEASE_SIZE + ' of space.')
        else:
            print('WARNING: You are downloading all ScanNet ' + RELEASE_NAME + ' scans of type ' + file_types[0])
        print('Note that existing scan directories will be skipped. Delete partially downloaded directories to \
              re-download.')
        print('***')
        print('Press any key to continue, or CTRL-C to exit.')
        # key = raw_input('')
