...
from tools.tsdf_fusion.fusion import TSDFVolume, get_view_frustum, meshwrite
from tools.simple_loader import ScanNetDataset, collate_fn
...
def process_with_single_worker(args, scannet_files):
    ...
    for scene in tqdm(scannet_files):
        ...
        for idx, (cam_pose, depth_im, _) in enumerate(dataloader):
            if idx % 100 == 0:
...
if __name__ == "__main__":
    all_proc = opts.n_proc * opts.n_gpu

    ray.init(num_cpus=all_proc * (opts.num_workers + 1), num_gpus=opts.n_gpu)

    if opts.dataset == 'scannet':
        if not opts.test:
            opts.data_path = os.path.join(opts.data_path, 'scans')
        else:
            opts.data_path = os.path.join(opts.data_path, 'scans_test')
        files = sorted(os.listdir(opts.data_path))
    else:
    ...
    for w_idx in range(all_proc):
        ray_worker_ids.append(process_with_single_worker.remote(opts, files[w_idx]))
