...
class ScanNetDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        depth_im = cv2.imread(os.path.join(self.data_path, self.scene, "depth", str(idx) + ".png"), -1).astype(
            np.float32)
        depth_im /= 1000.  # depth is saved in 16-bit PNG in millimeters
        depth_im[depth_im > self.max_depth] = 0

        # Read RGB image
        color_image = cv2.cvtColor(cv2.imread(os.path.join(self.data_path, self.scene, "color", str(idx) + ".jpg")),
                                   cv2.COLOR_BGR2RGB)
