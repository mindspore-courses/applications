...
from .modules_dense import ConvGRU
...
class GRUFusion(nn.Module):
    ...
    def update_map(self, value, coords, target_volume, valid, valid_target,
        ...
        self.global_volume[scale].F = torch.cat(
            [self.global_volume[scale].F[not valid], value])
        coords = coords + relative_origin
        self.global_volume[scale].C = torch.cat([self.global_volume[scale].C[not valid], coords])

        # target
        if target_volume is not None:
            target_volume = target_volume.squeeze()
            self.target_tsdf_volume[scale].F = torch.cat(
                [self.target_tsdf_volume[scale].F[not valid_target],
            ...
            self.target_tsdf_volume[scale].C = torch.cat(
                [self.target_tsdf_volume[scale].C[not valid_target], target_coords])
    ...
    def forward(self, coords, values_in, inputs, scale=2, outputs=None, save_mesh=False):
        ...
        for i in range(batch_size):
            ...
            # previous feature coordinates within FBV from our backend map to get new feature coordinates
            ...
            if not self.direct_substitude:
                ...
                dim = (torch.Tensor(self.cfg.N_VOX).cuda() // 2 ** (self.cfg.N_LAYER - scale - 1)).int()
                dim_list = dim.data.cpu().numpy().tolist()
                coords_tmp = generate_grid(self.cfg.N_VOX, interval)[0].long().permute(1, 0).contiguous() // interval

                h = sparse_to_dense_channel(coords_tmp, global_values, dim_list, self.ch_in[scale],
                                            0, global_values.device)
                x = sparse_to_dense_channel(coords_tmp, values, dim_list, self.ch_in[scale],
                                            0, global_values.device)

                h = h.permute(3, 0, 1, 2).unsqueeze(0).contiguous()
        ...
        if not self.direct_substitude:
            outputs = (updated_coords_all, values_all, tsdf_target_all, occ_target_all)
        return outputs
