...
class MnasMulti(nn.Module):
    ...
    def __init__(self, alpha=1.0):
        ...
        self.conv0 = nn.Sequential(
            mnasnet.layers._modules['0'],   # pylint: disable-protected-access
            mnasnet.layers._modules['1'],   # pylint: disable-protected-access
            mnasnet.layers._modules['2'],   # pylint: disable-protected-access
            mnasnet.layers._modules['3'],   # pylint: disable-protected-access
            mnasnet.layers._modules['4'],   # pylint: disable-protected-access
            mnasnet.layers._modules['5'],   # pylint: disable-protected-access
            mnasnet.layers._modules['6'],   # pylint: disable-protected-access
            mnasnet.layers._modules['7'],   # pylint: disable-protected-access
            mnasnet.layers._modules['8'],   # pylint: disable-protected-access
        )
...
if __name__ == '__main__':
    model = MnasMulti().cuda()

    input_tensor = torch.Tensor(np.random.randint(0, 10, size=(1, 3, 640, 480)).astype(np.float32)).cuda()
