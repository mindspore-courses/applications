...
from iconlib.dataset.mesh_util import SMPLX, get_visibility

import iconlib.common.render_utils as util
import torch
import numpy as np
...
def query_color(verts, faces, image, device):
    ...
    (x_y, z) = verts.split([2, 1], dim=1)
    visibility = get_visibility(x_y, z, faces[:, [0, 2, 1]])
    u_v = x_y.unsqueeze(0).unsqueeze(2)  # [B, N, 2]
    u_v = u_v * torch.tensor([1.0, -1.0]).type_as(u_v)
    colors = torch.nn.functional.grid_sample(
        image, u_v, align_corners=True
    )  # [B, C, N, 1]
    colors = (
...
class Render:
    ...
    def get_camera(self, cam_id):
        """get camera"""
        r_p, t_p = look_at_view_transform(
    ...
    def init_renderer(self, camera, type_p="clean_mesh", b_g="gray"):
        """init renderer"""
        if "mesh" in type_p:
        ...
        if type_p == "clean_mesh":

            self.renderer = MeshRenderer(
                rasterizer=self.mesh_ras,
                shader=CleanShader(
                    cameras=camera, blend_params=blendparam
                ),
            )
    ...
    def load_mesh(self, verts, faces, verts_rgb, verts_dense=None, load_seg=False):
        ...
        if not torch.is_tensor(verts):
            ...
            if verts_rgb is not None:
                verts_rgb = verts_rgb[:, :3].float().unsqueeze(0).to(self.device)
        ...
        self.mesh = Meshes(verts=verts, faces=faces, textures=textures).to(self.device)
    ...
    def load_simple_mesh(self, verts, faces, deform_verts=None):
        ...
        if deform_verts is not None:
            ...
            self.mesh = (
                Meshes(verts, faces).to(self.device).offset_verts(deform_verts_copy)
            )
        else:
    ...
    def get_clean_image(self, cam_ids=None):
        """get clean image"""
        if cam_ids is None:
            cam_ids = [0, 2]
        images = []
        for cam_id in cam_ids:
            if cam_id in cam_ids:
                ...
                if len(cam_ids) == 4:
                    rendered_img = (
                        self.renderer(self.mesh)[0:1, :, :, :3].permute(0, 3, 1, 2)
                        - 0.5
                    ) * 2.0
                else:
                    rendered_img = (
                        self.renderer(self.mesh)[0:1, :, :, :3].permute(0, 3, 1, 2) - 0.5
                    ) * 2.0
    ...
    def get_rendered_video(self, images, save_path):
        ...
        video = cv2.VideoWriter(
            save_path, fourcc, 30, (self.size + new_shape[1] * len(images), self.size)
        )
    ...
    def get_silhouette_image(self, cam_ids=None):
        """get_silhouette_image"""
        if cam_ids is None:
            cam_ids = [0, 2]
        images = []
        for cam_id in range(len(self.cam_pos)):
    ...
    def get_texture(self, smpl_color=None):
        ...
        if self.type == "dense":
            face_vertices = util.face_vertices(
                self.smpl_cmap[None, ...], self.faces.expand(batch_size, -1, -1)
            )
        ...
        uv_vertices_cmap = uv_vertices_cmap.squeeze(0).permute(1, 2, 0).cpu().numpy()
