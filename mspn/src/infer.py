...
def mspn_infer(args_opt):
    ...
    visualize(results=results,
              infer_dir=args_opt.data_url,
              save_dir=args_opt.img_save_dir,
              score_thre=args_opt.score_thresh)


def parse_args():
    """Parse MSPN infer arguments."""
    parser = argparse.ArgumentParser(description='MSPN infer.')
    parser.add_argument('--device_target', type=str, default="GPU", choices=["Ascend", "GPU", "CPU"])
    parser.add_argument('--device_id', type=int, default=7)
    parser.add_argument('--data_url', required=False, type=str, default="./infer_img", help='Data Dir Path')
    parser.add_argument('--det_file_url', required=False, type=str, default="./annotation/det_json/test.json",
                        help='Detection Path')
    parser.add_argument('--ckpt_path', type=str, default="./mspn.ckpt", help='Location of model ckpt.')
    parser.add_argument('--img_save_dir', type=str, default="./res_img", help='Prediction save dir')
    ...
    parser.add_argument('--score_thresh', type=float, default=0.7, help='MSPN Keypoints Confidence Score Thresh')

    return parser.parse_known_args()[0]


if __name__ == '__main__':
    mspn_infer(parse_args())
