...
class EvaluateCallBack(Callback):
    ...
    def __init__(self, args, model, eval_dataset, src_url, train_url, total_epochs, save_freq=50):
    ...
    def epoch_end(self, run_context):
        ...
        if cb_params.cur_epoch_num > self.total_epochs * 0.1:
            cur_epoch_num = cb_params.cur_epoch_num
            result = self.model.eval(self.eval_dataset)
            if result["acc"] > self.best_acc:
                self.best_acc = result["acc"]
            print(
                f'The best acc is {self.best_acc}, \
                        the current acc is {result["acc"]}')
