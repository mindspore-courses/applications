...
from models.loss import get_criterion, NetWithLoss, get_train_one_step
from models.convnext import convnext_tiny
from config.convnext_config import parse_args
from utils.tools import set_device, pretrained, get_dataset
from utils.cell import cast_amp
from utils.optimizer import get_optimizer
from utils.callback import EvaluateCallBack
...
def train(args):
    ...
    ckpoint_cb = ModelCheckpoint(prefix=args.arch + str(rank), directory=ckpt_save_dir,
                                 config=config_ck)
    loss_cb = LossMonitor()
    eval_cb = EvaluateCallBack(args, model, eval_dataset=data.val_dataset, src_url=ckpt_save_dir,
