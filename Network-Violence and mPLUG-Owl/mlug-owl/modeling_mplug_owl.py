...
import mindspore.nn as nnn
import warnings


class MplugOwlVisionEmbeddings(nnn.Cell):
    ...
    def construct(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
...
class MplugOwlVisionAttention(nnn.Cell):
...
class QuickGELU(nnn.Cell):
    def construct(self, x: torch.Tensor):
        return x * torch.sigmoid(1.702 * x)


class MplugOwlMLP(nnn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
    ...
    def construct(self, hidden_states: torch.Tensor) -> torch.Tensor:
...
class MplugOwlVisualAbstractorLayer(nnn.Cell):
...
class MplugOwlVisualAbstractorEncoder(nnn.Cell):
    def __init__(self, config):
        super().__init__()
        self.config = config
