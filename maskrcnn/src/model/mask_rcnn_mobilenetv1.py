...
from model.mobilenetv1 import MobileNetV1FeatureSelector
...
class MaskRcnnMobilenetv1(nn.Cell):
    """
    MaskRcnn Network.
    ...
    def __init__(self, config):
        super(MaskRcnnMobilenetv1, self).__init__()
        ...
        # Backbone mobilenetv1
        self.backbone = MobileNetV1FeatureSelector(1001, features_only=True).to_float(self.platform_mstype)
        # Fpn
        ...
        self.roi_align_mask = SingleRoIExtractor(config, config.roi_layer,
        ...
        self.roi_align_test = SingleRoIExtractor(config, config.roi_layer,
                                                 config.roi_align_out_channels,
                                                 config.roi_align_featmap_strides,
                                                 1, config.roi_align_finest_scale,
                                                 mask=False)
        self.roi_align_test.set_train_local(config, False)

        self.roi_align_mask_test = SingleRoIExtractor(config, config.roi_layer,
                                                      config.roi_align_out_channels,
    ...
    def construct(self, img_data, img_metas, gt_bboxes, gt_labels, gt_valids, gt_masks):
        """Construct Mask Rcnn Mobilenetv1"""
        ...
        if self.training:
            roi_feats = \
                self.roi_align(rois, self.cast(x[0], mstype.float32), self.cast(x[1], mstype.float32),
                               self.cast(x[2], mstype.float32), self.cast(x[3], mstype.float32))
        else:
            roi_feats = \
                self.roi_align_test(rois, self.cast(x[0], mstype.float32), self.cast(x[1], mstype.float32),
                                    self.cast(x[2], mstype.float32), self.cast(x[3], mstype.float32))
        ...
        if self.training:
            ...
            rcnn_mask_fb_loss = self.rcnn_mask(roi_feats_mask, rcnn_pos_labels,
                                               rcnn_pos_mask_squeeze,
                                               rcnn_pos_masks_fb)

            rcnn_loss = self.rcnn_loss_cls_weight * rcnn_cls_loss + \
                        self.rcnn_loss_reg_weight * rcnn_reg_loss + \
    ...
    def get_det_bboxes(self, cls_logits, reg_logits, mask_logits, rois, img_metas, mask_fb_pred_all):
        ...
        Args:
            cls_logits (list): Classification logits.
            reg_logits (list): Regression logits.
            mask_logits (list): Masks logits.
            rois (list): Regions of interest.
            img_metas (list): Image shapes.
            mask_fb_pred_all (list): All predicted feedback masks.

        Returns:
            Function, return a list of output tensor.
        """
    ...
    def init_tensors(self, config):
        ...
        roi_align_index_test = [np.array(np.ones((config.rpn_max_num, 1)) * i,
                                         dtype=self.platform_dtype) for i in range(self.test_batch_size)]
