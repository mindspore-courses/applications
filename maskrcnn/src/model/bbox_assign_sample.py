...
class BboxAssignSample(nn.Cell):
    ...
    Inputs:
        - **gt_bboxes_i** (tuple) - The ith ground truth bounding boxes.
        - **gt_labels_i** (bool) - The ith ground truth labels.
        - **valid_mask** (bool) - Validation maskes.
        - **bboxes** (tuple) - Bounding boxes.
        - **gt_valids** (tuple) - The ground truth validations.
    ...
    def construct(self, gt_bboxes_i, gt_labels_i, valid_mask, bboxes, gt_valids):
        """construction of bbox assign sample."""
        gt_valids_reshape = self.reshape(self.cast(gt_valids, mstype.int32), (self.num_gts, 1))
        gt_valids_target = self.cast(self.tile(gt_valids_reshape, (1, 4)), mstype.bool_)
        gt_bboxes_i = self.select(gt_valids_target, gt_bboxes_i, self.check_gt_one)

        bboxes_valid_reshape = self.reshape(self.cast(valid_mask, mstype.int32), (self.num_bboxes, 1))
        bboxes_valid_target = self.cast(self.tile(bboxes_valid_reshape, (1, 4)), mstype.bool_)
        bboxes = self.select(bboxes_valid_target, bboxes, self.check_anchor_two)
        ...
        max_overlaps_w_gt_inds = max_overlaps_w_gt_index + self.assigned_gt_ones
        assigned_gt_inds3 = self.select(pos_sample_iou_mask, max_overlaps_w_gt_inds, assigned_gt_inds2)
        ...
        pos_assigned_gt_index = self.gather_nd(assigned_gt_inds5, pos_index) - self.assigned_pos_ones
        ...
        pos_bboxes_ = self.gather_nd(bboxes, pos_index)
        pos_gt_bboxes_ = self.gather_nd(gt_bboxes_i, pos_assigned_gt_index)
        pos_gt_labels = self.gather_nd(gt_labels_i, pos_assigned_gt_index)
        ...
        bbox_targets_total = self.scatter_nd(pos_index, pos_bbox_targets_, (self.num_bboxes, 4))
        bbox_weights_total = self.scatter_nd(pos_index, valid_pos_index, (self.num_bboxes,))
        labels_total = self.scatter_nd(pos_index, pos_gt_labels, (self.num_bboxes,))
        total_index = self.concat((pos_index, neg_index))
        total_valid_index = self.concat((valid_pos_index, valid_neg_index))
        label_weights_total = self.scatter_nd(total_index, total_valid_index, (self.num_bboxes,))
