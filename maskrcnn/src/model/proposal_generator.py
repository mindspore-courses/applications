...
class Proposal(nn.Cell):
    ...
    Inputs:
        - **rpn_cls_score_total** (tuple) - Total rpn classification scores.
        - **rpn_bbox_pred_total** (tuple) - Total rpn bounding box predictions.
        - **anchor_list** (list) - Anchor list.
    ...
    Examples:
        >>> Proposal(config = config, batch_size = 2,
        ...          num_classes = 2, use_sigmoid_cls = True)
    """
    def __init__(self, config, batch_size, num_classes, use_sigmoid_cls,
    ...
    def set_train_local(self, config, training=True):
        ...
        Args:
            config(dict): Config.
            training(bool): Whether to train.
        """
        ...
        self.topk_v2 = P.TopK(sorted=True)
    ...
    def construct(self, rpn_cls_score_total, rpn_bbox_pred_total, anchor_list):
        """Construction of proposal generator."""
        proposals_tuple = ()
        masks_tuple = ()
    ...
    def get_bboxes_single(self, cls_scores, bbox_preds, mlvl_anchors):
        ...
        Args:
            cls_scores(list): Classification scores.
            bbox_preds(list): Bounding boxes prediction.
            mlvl_anchors(list): Mlvl anchors.

        Returns:
            Tensor, mulyiple tensors to output.
        """
        mlvl_proposals = ()
        mlvl_mask = ()
        for idx in range(self.num_levels):
            ...
            scores_sorted, topk_inds = self.topk_v2(rpn_cls_score_process, self.topk_stage1[idx])

            topk_inds = self.reshape(topk_inds, self.topk_shape[idx])

            bboxes_sorted = self.gather_nd(rpn_bbox_pred_process, topk_inds)
            anchors_sorted = self.cast(self.gather_nd(anchors, topk_inds), self.cast_type)
        ...
        _, topk_inds = self.topk_v2(scores_using, self.max_num)

        topk_inds = self.reshape(topk_inds, self.topk_shape_stage2)
        proposals = self.gather_nd(proposals, topk_inds)
        masks = self.gather_nd(masks, topk_inds)
