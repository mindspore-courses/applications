...
class BboxAssignSampleForRcnn(nn.Cell):
    ...
    def __init__(self, config, batch_size, num_bboxes, add_gt_as_proposals):
        ...
        self.check_neg_mask = Tensor(np.array(np.ones(self.num_expected_neg - self.num_expected_pos),
                                              dtype=np.bool))
    ...
    def construct(self, gt_bboxes_i, gt_labels_i, valid_mask, bboxes, gt_valids, gt_masks_i):
        """Construct Bbox assigner and sampler."""
        ...
        neg_sample_iou_mask = self.logicaland(self.greaterequal(max_overlaps_w_gt, self.scalar_zero),
        ...
        assigned_gt_inds5 = self.select(valid_mask, assigned_gt_inds3,
                                        self.assigned_gt_ignores)
        ...
        num_pos = self.sum_inds(self.cast(self.logicalnot(
            valid_pos_index), self.cast_type), -1)
        ...
        neg_bboxes_ = self.gather_nd(bboxes, neg_index)
        pos_assigned_gt_index = self.reshape(pos_assigned_gt_index, self.reshape_shape_pos)
        ...
        roi_pos_masks_fb = self.gather_nd(gt_masks_i, pos_assigned_gt_index)
        ...
        # convert gt masks targets be 0 or 1 to use
        ...
        return(total_bboxes, total_deltas, total_labels, total_mask,
               pos_bboxes_, pos_masks_fb, pos_gt_labels, valid_pos_index)
