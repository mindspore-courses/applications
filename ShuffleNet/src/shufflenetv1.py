...
class GroupConv(nn.Cell):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, pad_mode="pad", pad=0, groups=1, has_bias=False):
        ...
        self.groups = groups
        self.convs = nn.CellList()          # 相当于pytorch中的nn.ModuleList，和sequential的区别在于没实现forward方法
                                            # https://blog.csdn.net/BubbleCodes/article/details/124836575
        self.op_split = P.Split(axis=1, output_num=self.groups)     # 分割成groups组
        self.op_concat = P.Concat(axis=1)       # 拼接
        self.cast = P.Cast()                    # 转换数据类型
        for _ in range(groups):
            self.convs.append(nn.Conv2d(in_channels // groups, out_channels // groups,
                                        kernel_size=kernel_size, stride=stride, has_bias=has_bias,
                                        padding=pad, pad_mode=pad_mode, group=1, weight_init='xavier_uniform'))
            # 上面似乎就是暴力的整了groups组个卷积
    def construct(self, x):
        features = self.op_split(x)     # 将输入x按通道拆分成groups组
        outputs = ()
        for i in range(self.groups):
            outputs = outputs + (self.convs[i](self.cast(features[i], mstype.float32)),)    # '+': 元组的串接
        out = self.op_concat(outputs)   # 再拼起来
        return out


class ShuffleV1Block(nn.Cell):          # 相当于pytorch中的nn.Module
...
class ShuffleNetV1(nn.Cell):
    def __init__(self, n_class=1000, model_size='2.0x', group=3):
        ...
        self.classifier = nn.Dense(self.stage_out_channels[-1], n_class)    # 就是pytroch里的linear
        self.reshape = P.Reshape()                  # mindspore库里的reshape函数
    ...
    def construct(self, x):
        ...
        x = self.features(x)
        x = self.globalpool(x)
        x = self.reshape(x, (-1, self.stage_out_channels[-1]))  # 将其铺平，从原来1x1x960变成1x960
        x = self.classifier(x)      # 展开成向量后通过全连接层，输出为类别数
        return x
