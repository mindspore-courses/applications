...
def test():
    ...
    if config.is_transfer:
        _, dataset = create_flower_dataset()
            ...
            dataset = classification.dataset.Cifar10(path=config.eval_dataset_path, split='test', shuffle=False, batch_size=config.batch_size)
            dataset = dataset.run()
    #     dataset = create_dataset(config.eval_dataset_path, do_train=False, device_num=1, rank=0)
    # # step_size = dataset.get_dataset_size()
    # batches_per_epoch = ds_train.get_dataset_size()
