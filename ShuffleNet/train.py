...
import os
import time
from mindvision import classification
import mindspore
...
def train():
    ...
    if config.is_transfer:
        ...
        dataset = classification.dataset.Cifar10(path=config.train_dataset_path, split='train', shuffle=True,
    ...
    if config.resume:
        ...
        if config.is_transfer:
            ckpt.pop('classifier.weight')           # 为啥迁移学习这些就不要？
    ...
    if config.is_transfer:      # 相当于将网络，损失函数，优化器等都封装了
